...
uint8_t cpu6502::PLP()
{
	...
	SetFlag(U, 1);
	/*SetFlag(C, status & 0x01);
	SetFlag(Z, status & 0x02);
	SetFlag(I, status & 0x04);
	SetFlag(D, status & 0x08);
	SetFlag(B, status & 0x10);
	SetFlag(U, status & 0x20); //Unused
	SetFlag(V, status & 0x40);
...
//	pc = 0x4020;
//	pc = 0x0100;
	pc = (uint16_t)read(0xFFFE) | ((uint16_t)0xFFFF << 8);
...
//CPU Interrupt Request. Occurs if flag is set to 0.
void cpu6502::irq()
{
	if (GetFlag(I) == 0)
	{
		...
		//Push status registers to stack
		SetFlag(B, 0);
		SetFlag(U, 1);
		SetFlag(I, 1);
		write(0x0100 + stkp, status);
...
//Non-Maskable Interrupt from PPU. 
//Behaves like IRQ, while also reading PC address at 0xFFFA
void cpu6502::nmi()
{
	write(0x0100 + stkp, (pc >> 8) & 0x00FF);
	stkp--;
...
std::map<uint16_t, std::string> cpu6502::disassemble(uint16_t nStart, uint16_t nStop)
{
	...
	while (addr <= (uint32_t)nStop)
	{
		...
		//Define string for each addressing mode
		if (lookup[opcode].addrmode == &cpu6502::IMP)
		{
			instruction += " {IMP}";
		}
		...
		else if (lookup[opcode].addrmode == &cpu6502::ABY)
		{
			...
			hi = bus->cpuRead(addr, true); 
			addr++;
			instruction += "$" + hex((uint16_t)(hi << 8) | lo, 4) + ", Y {ABY}";
		}
		else if (lookup[opcode].addrmode == &cpu6502::IND)
		{
			...
			hi = bus->cpuRead(addr, true); 
			addr++;
			instruction += "($" + hex((uint16_t)(hi << 8) | lo, 4) + ") {IND}";
		}
		...
		mapLines[line_addr] = instruction;
	}
